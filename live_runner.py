"""
live_runner.py  -  2-Candle Momentum Strategy (with H1 Ranging Filter) - LIVE
==============================================================================
WARNING: This script trades with REAL money.
Always run demo_runner.py first until you are fully satisfied.
...
--------------------
    1. MT5 is open and logged in to your LIVE account
    2. AutoTrading is ENABLED (green robot icon)
    3. .env contains correct LIVE credentials
    4. Tested thoroughly on demo_runner.py
...
def _require(key: str) -> str:
    val = os.getenv(key)
    if not val:
        sys.exit(f"ERROR: '{key}' is not set in .env - aborting for safety.")
    return val
...
def main():
    ...
    def _shutdown(sig, frame):
        print("\nShutdown signal received. Closing open positions and stopping...")
        node.stop()
        time.sleep(3)
        sys.exit(0)
    ...
    while True:
        try:
            print("Connecting to MT5 live account...\n")
            node.build()
            node.run()
            break
